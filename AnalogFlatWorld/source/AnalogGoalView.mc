...
class AnalogGoalView extends Ui.View {
    ...
    function onLayout(dc) {
        if(theGoal == App.GOAL_TYPE_STEPS) {
            carIcon = new Ui.Bitmap({:rezId=>Rez.Drawables.RallyCarIcon,:locX=>dc.getWidth() / 2 - 75,:locY=>120});
        }
        else if(theGoal == App.GOAL_TYPE_FLOORS_CLIMBED) {
            carIcon = new Ui.Bitmap({:rezId=>Rez.Drawables.SportsCarIcon,:locX=>dc.getWidth() / 2 - 75,:locY=>120});
        }
        else if(theGoal == App.GOAL_TYPE_ACTIVE_MINUTES) {
            carIcon = new Ui.Bitmap({:rezId=>Rez.Drawables.PunchBuggyIcon,:locX=>dc.getWidth() / 2 - 70,:locY=>120});
        }
    }
